(function app() {
	...
	var view = {
		...
		typeChar: function typeChar(c) {
			...
			if (realChr != '') {
				...
				var pright = this.curPos == _p.length ?
				this.getCursor() :
		...
		moveCursor: function moveCursor() {var pos = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
			this.curPos = pos;
			this.setCursor();
		},
		initCursor: function initCursor() {var char = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "&nbsp;";
			this.removeCursor();
			this.curPos = 0;
			this.$history = false;
		...
		setCursor: function setCursor() {
			if (this.curPos >= 0) {
				...
				this.getCursor() :
				this.getCursor(_p3.substring(this.curPos, this.curPos + 1))) +
		...
		getCursor: function getCursor() {var char = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "&nbsp;";
		...
		printTerminal: function printTerminal(txt) {var cssClasses = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "command";
		...
		promptHistory: function promptHistory() {var prev = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
		...
		scrollPage: function scrollPage(direction) {var _this = this;
	...
	var controller = {
		...
		triggerCommand: function triggerCommand() {var prompt = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
			view.$prompt.trigger('command', this.getCommand(prompt));
		},
		executeCommand: function executeCommand() {var cmd = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
			var out = '';
			switch (cmd.command) {
				case '':
					break;
        ...
        case 'clear':
          ...
          out = "Type 'help' for more information.";
				...
				case 'exit':
					...
					break;
				case 'about':
					out = 'Made by Timothy Howard (<a href="https://timhow38/about.me">about.me</a>)<br>Copyright (c) 2018 Timothy Howard';
					break;
				case '?':
				case 'h':
				case 'help':
					out = '<table style="width:20%"><tr><th><u>Command</u></th><th><u>Description</u></th></tr><tr><td>cls</td><td>Clear Terminal</td></tr><tr> <td>about</td><td>About Creator</td></tr><tr> <td>help</td><td>Command List</td></tr><tr> <td>calc</td><td>Simple Math</td></tr><tr> <td>search</td><td>Google Phrase</td></tr><td>web</td><td>Web Search</td></tr><tr> <td>rss</td><td>RSS Search</td></tr><tr> <td>exit</td><td>Exit Terminal</td></tr></table>';
					break;
				case 'eval':
				case 'calc':
		...
		getCommand: function getCommand() {var prompt = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
			var arrPrompt = prompt.split(' ');
			return {
				'command': arrPrompt.shift().toLowerCase(),
				'arguments': arrPrompt.filter(function (arg) {return arg.length > 0;}) };

		},
		getFeedArticles: function getFeedArticles(json) {var _this2 = this;
			var out = '';
			try {
				if ($.isArray(json.query.results.item)) {
					$.each(json.query.results.item, function (i, item) {
						out = out + '<a href="' + item.link + '" title="' + _this2.encodeHtmlEntity(item.description) + '">' + item.title + '</a><br>';
					});
				} else {
		...
		getFeedYQL: function getFeedYQL(url) {
			var yql = 'https://query.yahooapis.com/v1/public/yql?q=select%20title%2Clink%2Cdescription%20from%20rss%20where%20url%3D%22' + encodeURI(url) + '%3Fformat%3Dxml%22&format=json&diagnostics=true&callback=';
